...
export const musicPlayerLink = readonly({
  /** 点击播放按钮的监听 */
  onRequestPlay: (listen: () => void) => {
    onRequestEventListens.onRequestPlay = listen;
    return () => {
      if (onRequestEventListens.onRequestPlay === listen) {
        onRequestEventListens.onRequestPlay = createEventListens().onRequestPlay;
      }
    };
  },
  /** 点击暂停按钮的监听 */
  onRequestPause: (listen: () => void) => {
    onRequestEventListens.onRequestPause = listen;
    return () => {
      if (onRequestEventListens.onRequestPause === listen) {
        onRequestEventListens.onRequestPause = createEventListens().onRequestPause;
      }
    };
  },
  /** 请求音量 */
  onRequestVolume: (listen: (volume: number) => void) => {
    onRequestEventListens.onRequestVolume = listen;
    return () => {
      if (onRequestEventListens.onRequestVolume === listen) {
        onRequestEventListens.onRequestVolume = createEventListens().onRequestVolume;
      }
    };
  },
  /** 请求当前播放位置 */
  onRequestCurrentTime: (listen: (currentTime: number) => void) => {
    onRequestEventListens.onRequestCurrentTime = listen;
    return () => {
      if (onRequestEventListens.onRequestCurrentTime === listen) {
        onRequestEventListens.onRequestCurrentTime = createEventListens().onRequestCurrentTime;
      }
    };
  },
  /** 更新当前播放位置 */
